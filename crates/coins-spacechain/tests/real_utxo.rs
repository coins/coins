use coins_spacechain::{Spacechain, inscribe, broadcast::{RestBroadcaster, Broadcaster}};
...
async fn real_utxo_flow() -> anyhow::Result<()> {
    ...
    // build minimal chain
    let (sc, _) = Spacechain::generate(1, OutPoint::null(), Amount::from_sat(546), Network::Signet);
