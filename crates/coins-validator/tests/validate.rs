use coins_crypto::{SecretKey, G1, aggregate, sign};
...
fn validate_single_tx() {
    ...
    let sender_acct = state.create_account(G1(sender_sk.public_key())).unwrap();
    let recipient_sk = SecretKey::random();
    let recipient_acct = state.create_account(G1(recipient_sk.public_key())).unwrap();
    let agg_sk = SecretKey::random();
    let _agg_acct = state.create_account(G1(agg_sk.public_key())).unwrap();
    ...
    let sb = SubBlock { sigma, aggregator_pk: G1(agg_sk.public_key()), txs: vec![tx] };
    ...
    let agg_new = state.get_by_pk(&G1(agg_sk.public_key())).unwrap().unwrap();
