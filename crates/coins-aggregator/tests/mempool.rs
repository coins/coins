...
async fn valid_tx_gets_enqueued() {
    ...
    let mut sender = state.create_account(pk).unwrap();
    ...
    let tx = Transaction { sender_id: sender.id.0, recipient_pk: r_pk, amount: 100, fee: 1 };
    ...
    let tx2 = Transaction { sender_id: sender.id.0, recipient_pk: r_pk, amount: 200, fee: 1 };
...
async fn unknown_sender_rejected() {
    ...
    let tx = Transaction { sender_id: 999, recipient_pk: r_pk, amount: 1, fee: 1 };
...
async fn insufficient_balance_rejected() {
    ...
    let sk = SecretKey::random();
    let pk = sk.public_key();
    let mut acct = state.create_account(pk).unwrap();
    acct.balance = 10; // 10 sats
    state.insert_account(&acct).unwrap();

    let mp = Mempool::new(state);
    let r_pk = SecretKey::random().public_key();
    let tx = Transaction { sender_id: acct.id.0, recipient_pk: r_pk, amount: 20, fee: 1 };
...
async fn preserves_insertion_order() {
    ...
    let mp = Mempool::new(state.clone());
    let r_pk = SecretKey::random().public_key();
    let tx1 = Transaction { sender_id: acct.id.0, recipient_pk: r_pk, amount: 10, fee: 1 };
    let tx2 = Transaction { sender_id: acct.id.0, recipient_pk: r_pk, amount: 20, fee: 1 };
...
async fn nonce_is_incremented() {
    ...
    // first tx
    let tx1 = Transaction { sender_id: sender.id.0, recipient_pk: r_pk, amount: 50, fee: 1 };
    mp.validate_and_enqueue(tx1).await.unwrap();
    assert_eq!(mp.nonce_of(sender.id).await, Some(1));

    // second tx
    let tx2 = Transaction { sender_id: sender.id.0, recipient_pk: r_pk, amount: 70, fee: 1 };
...
async fn refresh_removes_executed_txs() {
    ...
    // two txs queued
    let tx1 = Transaction { sender_id: sender.id.0, recipient_pk: r_pk, amount: 100, fee: 1 };
    let tx2 = Transaction { sender_id: sender.id.0, recipient_pk: r_pk, amount: 150, fee: 1 };
